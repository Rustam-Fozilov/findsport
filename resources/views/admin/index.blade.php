            ...
            <div class="card card-border">
                ...
                <div class="card-body nopadmar users">
                    ...
                    <div class="media">
                        <div>
                            @if($user?->avatar?->url ?? '')
                            <img src="{{ $user->avatar->url ?? ''}}" class="media-object rounded-circle">
                            @else
...
<script>
    ...
    <!-- prettier-ignore -->
    {{--var demo = new CountUp("clubs", 12.52, {{$clubs->count()}}, 0, 6, options);--}}
    demo.start();
    <!-- prettier-ignore -->
    {{--var demo = new CountUp("events", 1, {{ $events->count() }}, 0, 6, options);--}}
    demo.start();
    <!-- prettier-ignore -->
    var demo = new CountUp("my_visitors", 24.02, {{ $visitors }}, 0, 6, options);
    demo.start();
    <!-- prettier-ignore -->
    var demo = new CountUp("playgrounds", 10, {{ $grounds->count() }}, 0, 6, options);
    demo.start();
    <!-- prettier-ignore -->
    {{--var demo = new CountUp("sections", 10, {{ $sections->count() }}, 0, 6, options);--}}
    demo.start();
